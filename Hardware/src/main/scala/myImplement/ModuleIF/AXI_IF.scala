...
class AXI_IF(memAddrWidth: Int, memDataWidth: Int, busDataWidth: Int) extends Module {
  ...
  io.memIF.rdata := Cat(
    0.U((memDataWidth - busDataWidth).W),
    io.bus_master.readData.bits.data
  ) // lower 32 bits are values
  ...
  val sNormal :: sAXIReadSend :: sAXIReadWait :: sAXIWriteSendAddr :: sAXIWriteSendData :: sAXIWriteWait :: Nil = Enum(
    6
  )
