...
class DataMem_AXI(size: Int, addrWidth: Int, dataWidth: Int, binaryFile: String) extends Module {
  ...
  switch(stateReg) {
    is(sIdle) {
      addrReg := Mux(
        io.bus_slave.readAddr.valid,
        io.bus_slave.readAddr.bits.addr - "h8000".U,
    ...
    is(sWriteData) {
      ...
      addrReg := Mux(
        io.bus_slave.writeData.valid & ~io.bus_slave.writeData.bits.last,
        addrReg + 4.U(addrWidth.W),
        addrReg
      )
    }
    is(sWriteResp) {
