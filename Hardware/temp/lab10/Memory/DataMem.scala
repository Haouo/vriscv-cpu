  ...
  when(io.wen) {
    ...
    srdata := MuxLookup(
      ...
      Seq(
        ...
        Half  -> Cat(memory((io.raddr & (~(1.U(bits.W)))) + 1.U), memory(io.raddr & (~(1.U(bits.W))))).asSInt,
        Word  -> Cat(
          memory((io.raddr & ~(3.U(bits.W))) + 3.U),
          memory((io.raddr & ~(3.U(bits.W))) + 2.U),
          memory((io.raddr & ~(3.U(bits.W))) + 1.U),
          memory(io.raddr & ~(3.U(bits.W)))
        ).asSInt,
        UByte -> Cat(0.U(24.W), memory(io.raddr)).asSInt,
        UHalf -> Cat(0.U(16.W), memory((io.raddr & ~(1.U(bits.W))) + 1.U), memory(io.raddr & ~(1.U(bits.W)))).asSInt
      )
    )
  }
}
